...
output_model_file = PROJ_DIR + '/output/weights/autoencoder_mri_kf.h5'
# annotation_file = PROJ_DIR + '/output/annotation/annotations.tsv'
output_model_weight_file = PROJ_DIR + '/output/weights/autoencoder_weight_mri_kf.h5'
...
def l1_loss(x, v, c=count):
    ...
    print(count)
    # print(v.shape)
    return K.mean(losses.mean_absolute_error(K.flatten(x), K.flatten(v[c])))
...
class CustomVariationalLayer(layers.Layer):
    ...
    def __call__(self, inputs):
        ...
        v, _, _ = normalization(text_embedding)
        loss = l1_loss(inputs, v)
        self.add_loss(loss, inputs=inputs)
        return inputs


def autoencoder(input_img, regularizer=None):
    # encoder
    ...
    latent = Conv3D(data_format='channels_last', filters=1, kernel_size=(2, 2, 2), strides=stride, activation='relu',
                    padding=padding, name='latent', kernel_regularizer=regularizer)(x)
    latent = CustomVariationalLayer()(latent)
    return latent
...
def create_model(re):
    ...
    latent = autoencoder(input_img, regularizer=keras.regularizers.l1(re))
    reconstruction = autodecoder(latent)
    # autoencoder = Model(inp,reconstruction)
    # latent, output_img = autoencoder(input_img)
...
def main():
    ...
    # for i_reg in params: 5 0.01
    auto_encoder = create_model(re=0.01)
    # cross validation
    for i in range(n_splits):
        ...
        train_x = np.array(train).reshape([-1, IMAGE_HEIGHT, IMAGE_WIDTH, IMAGE_DEPTH, 1])
        train_x, _, _ = normalization(train_x)
        valid_x = np.array(valid).reshape([-1, IMAGE_HEIGHT, IMAGE_WIDTH, IMAGE_DEPTH, 1])
        ...
        mse = autoencoder_train.history['mse']
        val_mse = autoencoder_train.history['val_mse']
        loss_dic[i] = loss
        acc_dic[i] = mse
